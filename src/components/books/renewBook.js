...
const RenewBook = (props) => {
	...
	const onFinish = (values) => {
		...
		var config = {
			method: 'post',
			url: 'http://localhost:8080/api/book/transaction/renew/' + values.bookRfID,
			headers: {},
			data: data
		};
		...
		axios(config)
			.then(response => {
				...
				console.log(error)
				setMessage('Could not complete renewal:' + error && error.response && error.response.data ? error.response.data.error : 'Unknown error occured!')
				setModal(true)
			})
